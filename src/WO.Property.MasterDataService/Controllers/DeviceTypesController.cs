...
public class DeviceTypesController : ControllerBase
{
    ...
    public DeviceTypesController(MySqlConnection db, ILogger<DeviceTypesController> logger)
    {
        _db = db;
        _logger = logger;
    ...
    public async Task<IActionResult> GetAll([FromQuery] int page = 1, [FromQuery] int pageSize = 50, [FromQuery] string? status = null)
    {
        ...
        var countSql = $"SELECT COUNT(*) FROM DeviceTypes {where}";
        ...
        var dataSql = $"SELECT * FROM DeviceTypes {where} ORDER BY SortOrder, Id LIMIT @offset, @pageSize";
        ...
        var items = new List<DeviceTypeItem>();
        using var reader = await dataCmd.ExecuteReaderAsync();
        while (await reader.ReadAsync())
            items.Add(MapDeviceType(reader));
    ...
    {
        using var cmd = new MySqlCommand("SELECT * FROM DeviceTypes WHERE Id = @id", _db);
        cmd.Parameters.AddWithValue("@id", id);
        using var reader = await cmd.ExecuteReaderAsync();
        if (await reader.ReadAsync())
            return Ok(new { success = true, data = MapDeviceType(reader) });
        return NotFound(new { success = false, message = "设备类型不存在" });
    }

    [HttpPost]
    public async Task<IActionResult> Create([FromBody] DeviceTypeItem req)
    {
        using var checkCmd = new MySqlCommand("SELECT COUNT(*) FROM DeviceTypes WHERE Code = @code", _db);
        ...
        _logger.LogInformation("创建设备类型: {Code} ({Name})", req.Code, req.Name);
        return CreatedAtAction(nameof(GetById), new { id }, new { success = true, message = "设备类型创建成功", data = new { id } });
    }

    [HttpPut("{id}")]
    public async Task<IActionResult> Update(int id, [FromBody] DeviceTypeItem req)
    {
        using var checkCmd = new MySqlCommand("SELECT * FROM DeviceTypes WHERE Id = @id", _db);
        ...
        _logger.LogInformation("更新设备类型: {Id}", id);
        return Ok(new { success = true, message = "设备类型更新成功" });
    }

    [HttpDelete("{id}")]
    public async Task<IActionResult> Delete(int id)
    {
        using var cmd = new MySqlCommand("DELETE FROM DeviceTypes WHERE Id = @id", _db);
        cmd.Parameters.AddWithValue("@id", id);
        var affected = await cmd.ExecuteNonQueryAsync();
        if (affected == 0)
            return NotFound(new { success = false, message = "设备类型不存在" });
        _logger.LogInformation("删除设备类型: {Id}", id);
        return Ok(new { success = true, message = "设备类型已删除" });
    }

    private static DeviceTypeItem MapDeviceType(MySqlDataReader r) => new()
    {
