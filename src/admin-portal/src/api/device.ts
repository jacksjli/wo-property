...
const ENDPOINTS = {
  ...
  REPAIR: (deviceId: number) => `/api/tenant/devices/${deviceId}/repair`,
...
// 提交维修记录
export const submitDeviceRepair = async (deviceId: number, data: any) => {
  const response = await deviceApi.post(ENDPOINTS.REPAIR(deviceId), data);
  return response;
};

// 提交保养记录
export const submitDeviceMaintenance = async (deviceId: number, data: any) => {
