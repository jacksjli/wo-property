...
const currentAssignTicket = ref<any>(null)
const assignStaffList = ref<any[]>([])
const assignForm = ref({ assigneeId: null as number | null })

const openAssignDialog = (row: any) => {
  currentAssignTicket.value = row
  assignForm.value.assigneeId = null
  authHttp.get('/api/users', { params: { pageSize: 100 } }).then((res: any) => {
    if (res.data?.success) {
      assignStaffList.value = res.data.data || []
    }
  }).catch(() => {
...
const handleAssign = async () => {
  if (!currentAssignTicket.value || !assignForm.value.assigneeId) {
    ElMessage.warning('请选择要指派的员工')
    return
  }
  try {
    await assignTicket(currentAssignTicket.value.id, assignForm.value.assigneeId)
...
<template>
  <div class="ticket-list">
    ...
    <div class="stats-grid">
      ...
      <el-card shadow="hover" class="stat-card" @click="handleStatClick('Open')">
        <div class="stat-content">
      ...
      <el-card shadow="hover" class="stat-card" @click="handleStatClick('Processing')">
        <div class="stat-content">
      ...
      <el-card shadow="hover" class="stat-card" @click="handleStatClick('Resolved')">
        <div class="stat-content">
    ...
    <el-dialog v-model="detailDialogVisible" title="工单详情" width="700px">
      <template v-if="viewingTicket">
        <el-descriptions :column="2" border>
          <el-descriptions-item label="工单编号">{{ viewingTicket.ticketCode }}</el-descriptions-item>
          ...
          <el-descriptions-item label="类型">{{ getTypeLabel(viewingTicket.category) }}</el-descriptions-item>
    ...
    <el-dialog v-model="assignDialogVisible" title="指派工单" width="500px">
      <el-form ref="assignFormRef" :model="assignForm" label-width="80px">
        <el-form-item label="工单">
          <span>{{ currentAssignTicket?.title }}</span>
        </el-form-item>
        <el-form-item label="指派给" prop="assigneeId">
          <el-select v-model="assignForm.assigneeId" placeholder="请选择员工" filterable style="width: 100%">
            <el-option
